...
import { keralaPackages, keralaBanners } from '../data/siteData';
...
const KeralaPackages = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return keralaPackages;
    return keralaPackages.filter(p => p.title.toLowerCase().includes(q));
  }, [searchTerm]);
        ...
        <div className="container mx-auto px-4">
          {filtered.length ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((pkg) => (
                <PackageCard key={pkg.id} pkg={pkg} category="kerala" />
              ))}
            </div>
          ) : (

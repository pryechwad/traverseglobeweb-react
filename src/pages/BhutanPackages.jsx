...
import { bhutanPackages, bhutanBanners } from '../data/siteData';
...
export default function BhutanPackages() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return bhutanPackages;
    return bhutanPackages.filter(p => p.title.toLowerCase().includes(q));
  }, [searchTerm]);
          ...
          <div className="absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 z-10 w-11/12 max-w-3xl">
            <div className="bg-white rounded-2xl p-4 md:p-5 shadow-2xl border border-lightGray">
              <div className="text-center mb-4">
                <h1 className="text-2xl md:text-3xl lg:text-4xl font-season font-bold text-darkBlue">Bhutan Holidays Packages</h1>
                <p className="text-darkBlue/70 mt-2 font-canva-sans">Discover the Last Shangri-La - Land of the Thunder Dragon</p>
              </div>
              <form
              ...
              >
                <input
                  ...
                  placeholder="Search Bhutan packages..."
                  value={query}
        ...
        <div className="container mx-auto px-4">
          {filtered.length ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((pkg) => (
                <PackageCard key={pkg.id} pkg={pkg} category="bhutan" />
              ))}
            </div>
          ) : (
